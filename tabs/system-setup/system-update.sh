    ...
    case ${PACKAGER} in
        pacman)
          ...
          $AUR_HELPER -S --needed --noconfirm rate-mirrors-bin
            ...
            if [ $? -ne 0 ] || [ ! -s /etc/pacman.d/mirrorlist ]; then
                echo "${RED}Rate-mirrors failed, restoring backup.${RC}"
        ...
        apt-get|nala)
            ...
            if ! command_exists nala; then
                $ESCALATION_TOOL apt-get install -y nala || { echo "${YELLOW}Falling back to apt-get${RC}"; PACKAGER="apt-get"; }
            fi
        ...
        xbps-install)
            ...
            ;;
        *)
            echo "${RED}Unsupported package manager: $PACKAGER${RC}"
            exit 1
            ;;
    esac
}
