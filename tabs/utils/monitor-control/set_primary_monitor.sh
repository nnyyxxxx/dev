#!/bin/sh -e

. ./utility_functions.sh

# Function to set a monitor as primary
set_primary_monitor() {
    monitor_list=$(detect_connected_monitors)
    IFS=$'\n' read -r -a monitor_array <<<"$monitor_list"

    clear
    echo "${BLUE}=========================================${RESET}"
    echo "${BLUE}  Set Primary Monitor${RESET}"
    echo "${BLUE}=========================================${RESET}"
    echo "${YELLOW}Choose a monitor to set as primary:${RESET}"
    for i in "${!monitor_array[@]}"; do
        echo "$((i + 1)). ${CYAN}${monitor_array[i]}${RESET}"
    done

    read -p "Enter the number of the monitor: " monitor_choice

    if ! [[ "$monitor_choice" =~ ^[0-9]+$ ]] || (( monitor_choice < 1 )) || (( monitor_choice > ${#monitor_array[@]} )); then
        echo "${RED}Invalid selection.${RESET}"
        return
    fi

    monitor_name="${monitor_array[monitor_choice - 1]}"

    if confirm_action "Set ${CYAN}$monitor_name${RESET} as the primary monitor?"; then
        echo "${GREEN}Setting $monitor_name as primary monitor${RESET}"
        execute_command "xrandr --output $monitor_name --primary"
        echo "${GREEN}Monitor $monitor_name set as primary successfully.${RESET}"
    else
        echo "${RED}Action canceled.${RESET}"
    fi
}

# Call the set_primary_monitor function
set_primary_monitor
